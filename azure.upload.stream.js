...
Azure.Client.prototype.createReadStream = function (params, options) {
    ...
    // Ask Azure to return Buffer instance
    params.buffer = true;
    try {
...
Azure.Client.prototype.createWriteStream = function (params, options) {
    ...
    //Handler to receive data and upload it to Azure.
    writeStream._write = function (chunk, enc, next) {
        ...
        // has ended then flush the chunk buffer downstream to Azure via the multipart
        // upload API.
        if (currentChunk.length > maxChunkSize) {

            // Make sure we only run when the Azure upload is ready
            runWhenReady(function () { flushChunk(next, false); });
        } else {

            // We dont have to contact Azure for this
            runWhenReady(next);
        }
    };
    ...
    writeStream.on('error', function () {
        if (uploadCursor) {
            if (FS.debug) {
                console.log('Azure cursor - ERROR!');
            }
        }
    });
    ...
    var flushChunk = function (callback, lastChunk) {
        ...
        self.resumableUploadStep(uploadingChunk, uploadCursor, function (err, result) {
            ...
            if (err) {
                ...
                // finished uploading all that data to Azure. So tell Azure to assemble those
